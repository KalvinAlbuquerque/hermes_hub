...
function NotificationsLogPage() {
  ...
  const [filters, setFilters] = useState({
    subject: '', templateId: '', clienteId: '', status: '', submittedByUserId: '', startDate: '', endDate: '',
  });
  ...
  const fetchLogs = async (page = 1, currentFilters = filters) => {
    try {
      setLoading(true);
      const params = new URLSearchParams({ page: page.toString(), pageSize: '15', ...currentFilters }).toString();
  ...
  const handleClearFilters = () => {
    const cleared = { subject: '', templateId: '', clienteId: '', status: '', submittedByUserId: '', startDate: '', endDate: '' };
    setFilters(cleared);
    fetchLogs(1, cleared);
  };
        ...
        {selectedLog && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><h3 className="text-sm font-medium text-muted-foreground">Clientes de Destino</h3><p>{selectedLog.clientes.map(c => c.name).join(', ')}</p></div>
            </div>
