// frontend/src/app/management/profiles/page.tsx
"use client";
...
// Organização das permissões por categoria
const permissionGroups = [
  ...
  },
  {
    title: 'Notificações',
    permissions: [
      { id: 'notifications:send', label: 'Enviar' },
      { id: 'notifications:approve', label: 'Aprovar' },
    ]
  },
  {
    title: 'Sistema',
    permissions: [
      ...
      { id: 'system:settings', label: 'Gerenciar Configurações' },
    ]
  }
];
...
function ManageProfilesPage() {
  ...
  const handleOpenModal = (profile: Profile | null) => {
    if (profile) {
      ...
      const initialPermissions = permissionGroups.flatMap(g => g.permissions).reduce((acc, p) => ({ ...acc, [p.id]: false }), {});
  ...
  const handlePermissionChange = (permissionId: string) => {
    const isChecked = !formData.permissions[permissionId];
    const newPermissions = { ...formData.permissions, [permissionId]: isChecked };
    const basePermission = permissionId.split(':')[0];

    if (isChecked && (permissionId.endsWith(':create') || permissionId.endsWith(':update') || permissionId.endsWith(':delete') || permissionId.endsWith(':write'))) {
      newPermissions[`${basePermission}:read`] = true;
    }
    ...
    <DashboardLayout>
      ...
      <Modal title={editingProfileId ? "Editar Perfil" : "Criar Novo Perfil"} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <form onSubmit={handleFormSubmit}>
          ...
          <div className="mb-4">
            <h3 className="block text-sm font-medium text-muted-foreground mb-3">Permissões</h3>
            <div className="space-y-4">
              {permissionGroups.map(group => (
                <div key={group.title}>
                  <p className="font-semibold text-foreground text-sm mb-2">{group.title}</p>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {group.permissions.map(p => {
                      const basePermission = p.id.split(':')[0];
                      const isRead = p.id.endsWith(':read');
                      const hasWriteOrDelete = formData.permissions[`${basePermission}:write`] || formData.permissions[`${basePermission}:delete`] || formData.permissions[`${basePermission}:create`] || formData.permissions[`${basePermission}:update`];
                      const isDisabled = isRead && hasWriteOrDelete;
